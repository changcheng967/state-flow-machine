...
_TOOLS = {
    ...
    "database": {
        "desc": "Queries structured databases",
        "actions": [
            ("query({sql})", lambda: f"Returned {random.randint(1,100)} rows"),
    ...
    "text_analysis": {
        "desc": "Analyzes and processes text",
        "actions": [
...
def gen_single_step_trajectory() -> str:
    ...
    tool_name = random.choice(list(_TOOLS.keys()))
    tool = _TOOLS[tool_name]
    action_template, obs_fn = random.choice(tool["actions"])
...
def gen_multi_step_trajectory(num_steps: int = 3) -> str:
    ...
    tools_used = random.sample(list(_TOOLS.keys()), min(num_steps, len(_TOOLS)))
    ...
    for step, tool_name in enumerate(tools_used):
        ...
        action = action_template.format(
            ...
            query="'search query'", term="'key term'",
            item="'target item'", id=random.randint(1, 1000),
            table="'data_table'",
            condition="'filter_condition'",
            path="'/path/to/file'", content="'data'",
            url="'/api/endpoint'", body="'{key: value}'",
            text="'sample text'", target_lang="'target'",
        )

        # Chain-of-thought: reference previous step
        if step == 0:
            thought = f"I need to start by using {tool_name} to get initial data."
        else:
            thought = (f"From the previous step, I now need to {random.choice(['refine', 'combine', 'validate', 'transform'])} "
    ...
    lines.append(f"Thought: I now have all the information needed to answer.")
    lines.append(f"Answer: After {len(tools_used)} steps of analysis, the answer is clear. "
                 f"Using {', '.join(tools_used)}, I determined the result by combining information from each tool.")
...
def gen_trajectory(seed: int | None = None) -> str:
    ...
    num_steps = random.choices([1, 2, 3, 4, 5], weights=[0.25, 0.30, 0.25, 0.15, 0.05])[0]
    if num_steps <= 2:
